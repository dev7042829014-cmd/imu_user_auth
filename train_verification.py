...
# Determinism: cuBLAS reads CUBLAS_WORKSPACE_CONFIG when the CUDA context is
# created (on `import torch` / first CUDA op), so it MUST be set before that.
# Required for deterministic matmuls under torch.use_deterministic_algorithms.
...
def set_seed(seed: int):
    ...
    NOTE: cuDNN's GRU (RNN) backward has no fully-deterministic implementation,
    so with warn_only=True the GRU-based models (g2/g2a) keep a small amount of
    residual run-to-run nondeterminism. This is documented and characterised by
    the multi-seed protocol (report mean +/- std over seeds), not hidden."""
    os.environ["PYTHONHASHSEED"] = str(seed)
    random.seed(seed); np.random.seed(seed)
    torch.manual_seed(seed); torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.benchmark = False       # no autotuner (nondeterministic pick)
    torch.backends.cudnn.deterministic = True    # deterministic conv algorithms
    try:
        # warn_only: don't crash on ops without a deterministic kernel (cuDNN GRU).
        torch.use_deterministic_algorithms(True, warn_only=True)
    except TypeError:                            # older torch: no warn_only kwarg
        try:
...
def main(args):
    ...
    data = VerificationData(args.cache_dir, args.split_file,
                            ...
                            enroll_first=(not args.verify_first))
    ...
    # Seed the loader's generator and each worker so data ordering is reproducible.
...
def build_argparser():
    ...
    p.add_argument("--enroll_ratio", type=float, default=0.7,
                   help="fraction of the session used for enroll; rest = verify (val eval)")
    p.add_argument("--verify_first", action="store_true",
                   help="val eval: put VERIFY on the first contiguous region, ENROLL on the last")
