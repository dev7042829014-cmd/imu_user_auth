...
def build_model(name: str, hidden_size: int = 256, embed_dim: int = EMBED_DIM_DEFAULT):
    ...
    # S1 = pure Swin Transformer 1-D backbone; S2 = Swin + spectral/band fusion
    # (the transformer counterpart of B1/M1 and of G2 respectively). Self-contained
    # in swin1d.py. s?a = SupCon, s?b = margin loss, same as every other family.
    # Swin's INTERNAL width is fixed at 128 (config locked with the data owner);
    # hidden_size here would over-parameterise it, so it is not forwarded.
    if name in ("s1", "s1a", "s1b", "s2", "s2a", "s2b"):
        from swin1d import make_swin
        return make_swin(name, hidden_size=128, embed_dim=embed_dim)
    raise ValueError(
        f"build_model: '{name}' is not a deep model (use b1/b2/m1/m2/g2/s1/s2).")
